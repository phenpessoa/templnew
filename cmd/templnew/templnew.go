...
//	gonew rsc.io/quote
//
// templnew is a fork of gonew with the only change that it also edits .templ files.
package main
...
import (
	...
	"github.com/phenpessoa/templnew/internal/edit"
	"golang.org/x/mod/modfile"
	"golang.org/x/mod/module"
)

func usage() {
	fmt.Fprintf(os.Stderr, "usage: templnew srcmod[@version] [dstmod [dir]]\n")
	fmt.Fprintf(os.Stderr, "See https://pkg.go.dev/github.com/phenpessoa/templnew.\n")
	os.Exit(2)
}
...
func main() {
	...
	if err := json.Unmarshal(stdout.Bytes(), &info); err != nil {
		log.Fatalf(
			"go mod download -json %s: invalid JSON output: %v\n%s%s",
			srcMod,
			err,
			stderr.Bytes(),
			stdout.Bytes(),
	...
	filepath.WalkDir(info.Dir, func(src string, d fs.DirEntry, err error) error {
		...
		if strings.HasSuffix(rel, ".go") || strings.HasSuffix(rel, ".templ") {
